...
model=None; threshold=float(os.getenv("CHURN_THRESHOLD",".50"))
...
def startup():
    ...
    if MODEL_PATH.exists():
        model=joblib.load(MODEL_PATH); MODEL_LOADED.set(1)
    if THRESHOLD_PATH.exists():
        threshold=float(json.loads(THRESHOLD_PATH.read_text()).get("threshold",threshold))

@app.get("/health")
def health(): return {"status":"ok","service":"customer-retention-api"}

@app.get("/ready")
def ready():
    if model is None: raise HTTPException(503,"Model artifact is not loaded")
    return {"status":"ready","model_version":MODEL_VERSION,"threshold":threshold}

@app.get("/model")
def model_info():
    return {"model_loaded":model is not None,"model_version":MODEL_VERSION,"threshold":threshold}
...
def predict(request:PredictionRequest):
    ...
    try:
        ...
        return {"customer_id":request.customer_id,"churn_probability":probability,"churn_prediction":prediction,"at_risk":bool(prediction),"threshold":threshold,"model_version":MODEL_VERSION}
