...
def find_target_column(
    ...
    if requested:
        ...
        if requested not in columns:
            raise ValueError(f"Target column '{requested}' not found")
        return requested
...
def encode_target(series: pd.Series) -> pd.Series:
    ...
    if encoded.isna().any():
        values = series[encoded.isna()].drop_duplicates().tolist()[:10]
        raise ValueError(f"Unsupported target values: {values}")

    return encoded.astype(int)


def prepare_xy(df: pd.DataFrame, target: str | None = None):
    frame = normalize_columns(df)
    frame = frame.replace([float("inf"), float("-inf")], pd.NA)

    target_column = find_target_column(frame, target)
    y = encode_target(frame.pop(target_column))
